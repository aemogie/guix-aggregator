...
(define sss-sans-font "Adwaita Sans")

(define sss-mono-font "Adwaita Mono")
...
I split the Emacs configuration into several modules that you can optionally include/exclude, see ~lib/emacs/modules~. Here follow some explanations of each module:
...
Then proceed to clone SSS with ~git clone --recursive https://codeberg.org/jjba23/sss.git~ to your favourite location. This location will need to be added to ~per-host.scm~. You can skip the ~--recursive~ option if you are not interested in the wallpapers and other art.
