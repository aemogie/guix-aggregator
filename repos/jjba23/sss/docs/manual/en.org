...
later in order fix we need to perform following steps:
login with root user (you also can have root access over ssh using: ssh -p 2222 root@127.0.0.1 command)
