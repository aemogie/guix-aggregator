...
;; set file where certain "custom" configurations will go
...
(setopt visible-bell 1)

;; right click to open up menu
(context-menu-mode 1)

;; remove superfluous tool-bar
(tool-bar-mode -1)
...
(use-package dired
  ...
  (dired-mode . hl-line-mode) ; highlight selected line
  :custom
...
;; describe what key commands do before you finish typing them (e.g. "C-x", "C-c", etc.)
(use-package which-key
  :ensure t
  :config
  (which-key-mode 1))

;; allow you to repeat certain commands without re-doing the entire key-chord (try e.g. "C-x o")
(use-package repeat
  :config
  (repeat-mode 1))
...
;; display line numbers in "programm-y" modes
...
;; show a vertical pop-up of minibuffer completions (e.g. when searching for a file, or typing M-x)
(use-package vertico
  :ensure t
  :config
  (vertico-mode 1))

;; show descriptive annotations for completion candidates in vertico
...
;; if you highlight text and start typing, you will replace that text (like in most applications)
(use-package delsel
  :config
