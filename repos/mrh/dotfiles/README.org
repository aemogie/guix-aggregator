...
My system is not completely free however, as some of my hardware is non-functional without proprietary kernel blobs.
I hope that hardware manufacturers will turn away from this practice.
...
I run Guix System, but it can also be installed as a standalone package manager atop any GNU/Linux distribution.

There are separate directories containing the system configurations for each of my guix machines.
Each machine configuration defines the entire operating system, from system packages to users to file systems, and everything else!
They can be found at =mrh-guix/system/<machine-name>=.

My home configurations are similarly machine specific and can be found at =mrh-guix/home/<machine-name>=.
...
Finally my custom guix packages, which are either not available or more up to date than those in the standard guix repo, are available as a [[https://guix.gnu.org/manual/devel/en/html_node/Channels.html][guix channel]] at [[https://codeberg.org/mrh/guix-channel][codeberg.org/mrh/guix-channel]].

* Emacs
[[https://www.gnu.org/software/emacs][GNU Emacs]] is a text editor and lisp interpreter designed to maximize hackability and software freedom. It also serves as the primary text editor for the GNU system.

I use emacs for just about everything on my computer other than browsing the web and consuming audio/visual media. This includes editing source code, but also:
...
- [[https://www.gnus.org/][Gnus]] for usenet
...
My configuration can be found at =dot-config/emacs/config.org=.
Note that my =init.el= is simply the tangled elisp codeblocks from =config.org=.
